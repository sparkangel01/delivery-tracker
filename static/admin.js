(function () {
  ...
  document.getElementById('saveBtn').addEventListener('click', function () {
    var body = {
      ...
      pay_name: g('f_pay_name'),
      pay_bank: g('f_pay_bank'),
      pay_account: g('f_pay_account'),
      pay_other: g('f_pay_other'),
      pay_note: g('f_pay_note'),
      note: g('f_note')
    };
  ...
  function clearForm() {
    ...
    ['f_code', 'f_status', 'f_origin', 'f_destination', 'f_recipient',
     ...
     'f_images', 'f_note_big', 'f_pay_name', 'f_pay_bank',
     'f_pay_account', 'f_pay_other', 'f_pay_note', 'f_note']
  ...
  function editShipment(code) {
    fetch('/api/admin/shipment/' + encodeURIComponent(code) + '?password=' + encodeURIComponent(password))
      .then(function (r) {
      ...
      .then(function (d) {
        ...
        setVal('f_pay_name', d.pay_name);
        setVal('f_pay_bank', d.pay_bank);
        setVal('f_pay_account', d.pay_account);
        setVal('f_pay_other', d.pay_other);
        setVal('f_pay_note', d.pay_note);
        setVal('f_note', '');
        editingCode = code;
    ...
    fetch('/api/admin/shipments?password=' + encodeURIComponent(password))
      ...
      .then(function (list) {
        ...
        container.innerHTML = list.map(function (s) {
          var payIcon = s.pay_account ? ' 💳' : '';
          return '<div class="shipment-item"><div><strong>' + s.code + '</strong>' + payIcon +
            '<br><small>' + s.status + ' — ' + (s.destination || '') +
      ...
      .catch(function () {
        document.getElementById('shipmentList').innerHTML = '<p class="empty">Could not load</p>';
      });
  }
