(function () {
  ...
  function track() {
    ...
    fetch('/api/track/' + encodeURIComponent(code))
      ...
      .then(function (r) {
        if (!r.ok) {
          result.innerHTML = '<div class="error">❌ ' + esc(r.data.error || 'Not found') + '</div>';
          return;
        }
        render(r.data);
      })
  ...
  function render(d) {
    ...
    var noteBlock = d.note_big
      ? '<div class="big-note"><div class="big-note-label">📝 Important Note</div>' +
        esc(d.note_big) + '</div>'
      : '';
  ...
  window.openPay = function () {
    ...
    var box = document.getElementById('payDetails');
    ...
    fetch('/api/payment')
      ...
      .catch(function () {
        box.innerHTML = '<div class="error">Could not load payment details</div>';
      });
  };
